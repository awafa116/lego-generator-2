...
const pixelationElement = document.querySelector('#pixels-in-row');
...
const pixelateImage = (originalImage, pixelationFactor) => {
  ...
  if (pixelationFactor !== 0) {
    for (let y = 0; y < originalHeight; y += pixelationFactor) {
      const pixelsArr = [];
      for (let x = 0; x < originalWidth; x += Math.floor(originalWidth / pixelationFactor)) {
